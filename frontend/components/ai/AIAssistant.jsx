...
/**
 * Floating AI assistant. Personality comes from data/quotes.js so copy can
 * be swapped without touching this component. `sassy` (from useEasterEggs)
 * temporarily overrides the greeting after 5 rapid clicks.
 */
export function AIAssistant({ sassy, onAssistantClick }) {
  const { timeOfDay } = useTimeOfDay();
  const [open, setOpen] = useState(true);
  const greeting = useMemo(() => pickGreeting(timeOfDay), [timeOfDay]);
  const activeLine = sassy ? AI_EASTER_EGG_LINE : greeting;
  const { displayed } = useTypewriter(activeLine, 22, 300);
    ...
    <div className="fixed bottom-6 right-6 z-40 flex flex-col items-end gap-3">
      <AnimatePresence>
        {open && (
          ...
          >
            <GlassCard weather="clear_night" interactive={false} className="w-72 p-4">
              <button
              ...
              <p className="min-h-[2.5rem] font-mono text-sm text-foreground/90">
                {displayed}
      ...
      <motion.button
        ...
        className={cn(
          ...
          "bg-gradient-to-br from-aurora-cyan to-aurora-violet shadow-[0_0_30px_rgba(77,216,230,0.5)]"
        )}
